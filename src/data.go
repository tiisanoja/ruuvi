...
var mapWait map[string]int64 = make(map[string]int64)
...
func lockSensor(MAC string) bool {
	...
	mutex.Lock()
	if mapWait[MAC] > timestamp {
		mutex.Unlock()
		log.Printf("INFO: Skipping %s\n", MAC)
		return false
	}
	mapWait[MAC] = timestamp + int64(StoreDelay)
	mutex.Unlock()
	return true
}

// ParseRuuviData parses raw ruuvidata received from RuuviTag
func ParseRuuviData(dataManufactor []byte, company uint16) {

	data := make([]byte, 2)
	data = append(data, dataManufactor...)
	if company == 0x0499 {
		if len(data) > 25 {
			sensorFormat := data[2]
		...
		//fmt.Printf("Not a RuuviTag device. Data: %s\n", hex.EncodeToString(data))
	}
